...
exports.changePassword = async (req, res) => {
    try {
        ...
        if (await bcrypt.compare(currentPassword, user.password)) {
            ...
            const updatedUser = await User.findByIdAndUpdate(user._id, { password: hashedNewPassword }, { new: true }).populate("additionalDetails")
                .exec();
            //send mail notification
            try {
                await mailSender(user.email, "Password for your account has been updated", `password updated for the user ${updatedUser.firstName} ${updatedUser.lastName}`);
                return res.status(200).json({
                    success: true,
                    updatedUser,
